...
from src.data_sources.loaders import get_data_source_status, load_crypto_prices, load_prices


def main():
    print("Checking live/near-live sources...")
    try:
        stocks = load_prices(data_mode="live_auto")
        print(f"US equities rows: {len(stocks):,}")
    except Exception as exc:
        print(f"US equities failed: {exc}")

    try:
        crypto = load_crypto_prices(data_mode="live_auto")
